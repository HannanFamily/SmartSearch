...
' Simple bootstrap to create ModeConfig sheet/table and the Sootblower Location entry
...
Public Sub Bootstrap_ModeConfig()
    ...
    ' Ensure Sootblower Location row exists
    ...
    For Each r In lo.ListRows
        If Trim$(CStr(r.Range(colMode).Value)) = "Sootblower Location" Then
            Set found = r
            Exit For
        End If
    Next r
    ...
    MsgBox "ModeConfig bootstrapped. You can now run Ensure_ModeConfigEntry_SootblowerLocation without errors.", vbInformation
End Sub
